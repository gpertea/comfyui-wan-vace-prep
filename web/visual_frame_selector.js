...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            const vfsCallback = (result) => {
                const { video, widget } = result;

                // All post-discovery setup, extracted so it can be called from
                // either the synchronous path (video already connected) or the
                // deferred path (video connected after DOM re-attachment below).
                const proceed = () => {
                ...
                // The video element exists but isn't connected to the document yet.
                // Two distinct causes observed in the wild:
                //   (a) Tab switch / simple re-attach: widget.element is temporarily
                //       detached; it re-connects shortly and video.isConnected becomes true.
                //   (b) Subgraph exit: ComfyUI tears down and recreates widget DOM entirely.
                //       widget.element is replaced with a fresh element; the originally-
                //       discovered video is orphaned and will never connect. Detected by
                //       widget.element.isConnected becoming true on the NEW element.
                // 2s fallback is the Nodes 2.0 / Vue renderer signal — in that renderer the
                // video is never in the real document.
                console.debug(
                    "[VisualFrameSelector] Video not yet connected to document (widget.element.isConnected=%s); waiting for DOM attachment...",
                    widget.element.isConnected
                );
                let settled = false;
                const cleanup = () => {
                    connectedWatcher.disconnect();
                ...
                const settle = (connected) => {
                    ...
                    cleanup();
                    if (connected) {
                        console.debug("[VisualFrameSelector] Video connected after DOM attachment; proceeding.");
                        proceed();
                    } else {
                ...
                // Case (b): widget.element was replaced. Cancel and re-run discovery
                // so proceed() wires to the fresh element via vfsCallback.
                const checkReplacement = () => {
                    if (!widget.element?.isConnected) return false;
                    const freshVideo = widget.element.querySelector("video");
                    if (!freshVideo?.isConnected) return false;
                    if (settled) return false;
                    settled = true;
                    cleanup();
                    console.debug("[VisualFrameSelector] widget.element replaced with connected element; restarting discovery.");
                    findNativeVideoElement(node, abortController, vfsCallback);
                    return true;
                };
                // Observe the full document to catch widget containers attached anywhere.
                const connectedWatcher = new MutationObserver(() => {
                    if (video.isConnected) { settle(true); return; }
                    if (checkReplacement()) return;
                    console.debug("[VisualFrameSelector] connectedWatcher fired but video still not connected (widget.element.isConnected=%s)", widget.element.isConnected);
                });
                connectedWatcher.observe(document, { childList: true, subtree: true });
                // Polling fallback — catches re-attachment or replacement that the
                // MutationObserver misses (shadow DOM, in-progress mutation batch).
                const pollId = setInterval(() => {
                    if (video.isConnected) { settle(true); return; }
                    checkReplacement();
                }, 50);
                const settleTimeout = setTimeout(() => {
                    console.debug("[VisualFrameSelector] 2s timeout: video.isConnected=%s, widget.element.isConnected=%s", video.isConnected, widget.element.isConnected);
                    settle(false);
                }, 2000);
                abortController.signal.addEventListener("abort", () => settle(false));
